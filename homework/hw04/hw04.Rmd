...
```{r}
all_dates = tibble(date = seq(ydm("2017-01-01"), ydm("2021-31-12"), 1))
  
all_dates

anti_join(all_dates, ord)
```
...
```{r}
total_flights_per_time = ord2 %>% 
  group_by(time_of_day) %>% 
  summarize(total_flights = sum(n, na.rm = TRUE)) 

total_flights_per_time
```
...
ord4 = ord %>%
  group_by(time_of_day) %>%
  summarise(num_flights = sum(flights))
ord4

ggplot(ord4, aes(x=time_of_day, y=num_flights)) +
  geom_col(
    color = "steelblue4",
    fill = "skyblue1"
  ) +labs(x="Part of the Day", y="Number of flights") +ggtitle("Total number of flights in each time period")
```
...
prob5 = ord %>% 
  ## add code
  group_by(year, month) %>%
  summarise(avg_daily_passengers = mean(passengers))
...
ggplot(prob5, aes(x=month, y=avg_daily_passengers)) + 
  ...
  labs(x="Months", y="Average daily passenger counts") +
  ggtitle("Month vs Daily passengers") +
  scale_y_continuous(label = scales::comma) +
  facet_wrap(facets = vars(year), ncol = 1, strip.position = "right")
...
> The feature facet_wrap was very important here, because it allowed me to realize the difference of number of fluights per year.
> I can realize that in years 2020 and 2021 there were significantly less flights.
...
question7 = ord %>%
  ...
  group_by(date) %>%
  summarise(passengers_per_day = sum(passengers)) 


question7_2 = question7 %>%
  mutate(wday = wday(date)) %>%
  group_by(wday) %>%
  summarise(avg_per_day = mean(passengers_per_day)) %>%
  arrange(desc(avg_per_day))
...
question8 = question7 %>%
  mutate(wday = wday(date, label = TRUE)) %>%
  slice_max(passengers_per_day, n=20) %>%
  select(date, wday, passengers_per_day)
question8

table(question8$wday)

solution = sum(question8["wday"] == "Sat") ## from previous question we know that Sat is the most active day
solution

##Only one date of this 20 is not saturday, so the answer is 1.
```
