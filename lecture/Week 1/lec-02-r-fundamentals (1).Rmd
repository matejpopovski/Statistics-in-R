...
* We want the working directory to be *the location of the R Markdown file you are working with*.
    + In RStudio, set `Tools > Global Options > R Markdown > Evaluate Chunks in Directory` to "Document" and hit `Apply`.
...
> Post-lecture addendum: If you do not see this setting when you navigate to that location, it is okay, this setting is set to "Document" invisibly and there is no action needed from you.
...
# This chunk should output the location of this file.

getwd()
```
    
> If this line does NOT show you the location of the file, click on `Session > Set Working Directory > To Source File Location` and try again.
